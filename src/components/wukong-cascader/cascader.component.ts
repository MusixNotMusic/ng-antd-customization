...
@Component({
  ...
  selector: 'wukong-cascader, [wukong-cascader]',
  exportAs: 'wukongCascader',
  ...
  providers: [
    {
      ...
      useExisting: forwardRef(() => WukongCascaderComponent),
      multi: true
    },
    NzCascaderService
  ],
  host: {
    '[attr.tabIndex]': '"0"',
    '[class.wukong-cascader-lg]': 'nzSize === "large"',
    '[class.wukong-cascader-sm]': 'nzSize === "small"',
    '[class.wukong-cascader-picker-disabled]': 'nzDisabled',
    '[class.wukong-cascader-picker-open]': 'menuVisible',
    '[class.wukong-cascader-picker-with-value]': '!!inputValue',
    '[class.wukong-cascader-focused]': 'isFocused'
  }
})
export class WukongCascaderComponent implements NzCascaderComponentAsSource, OnInit, OnDestroy, ControlValueAccessor {
  @ViewChild('input') input: ElementRef;
  @ViewChild('menu') menu: ElementRef;
  @ViewChild(CdkConnectedOverlay) overlay: CdkConnectedOverlay;
  @ViewChildren(WukongCascaderOptionComponent) cascaderItems: QueryList<WukongCascaderOptionComponent>;
  ...
  set nzOptions(options: NzCascaderOption[] | null) {
    console.log('withOptions', options);
    this.cascaderService.withOptions(options);
    if(options) {
      this.cascaderService.initMultiValueStatus();
    }
  }
  ...
  constructor(
    ...
    renderer.addClass(elementRef.nativeElement, 'wukong-cascader');
    renderer.addClass(elementRef.nativeElement, 'wukong-cascader-picker');
    window.com = this;
  }
  ...
  onTriggerClick(): void {
    ...
    if (this.isActionTrigger('click')) {
      this.delaySetMenuVisible(!this.menuVisible, 50);
    }
    this.onTouched();
  }
  ...
  private delaySetOptionActivated(option: NzCascaderOption, columnIndex: number, performSelect: boolean): void {
    this.clearDelaySelectTimer();
    (this.delaySelectTimer as any) = setTimeout(() => {
